...
describe('SellerVault', async function () {
  ...
  it("[Signing Order Hash]", async function() {
    ...
    const chain_id = 0;
    const signerKey = "0x967b365ecbf05c4a7dc045e484d45259c5c3cfb57b8dc472ddf57e256881d0b2";
    ...
    // let nonce = await sellerVault.user_nonce(users[0].address);
    ...
    await usdc.approve(sellerVault.address, (10 ** 18).toString());
    
    // list_sell 내부에 orderFactory.create_escrow로 보내어주는 hyperLane 추가
    // sellerVault: 외부 체인 (Unstable 배포)
    // orderFactory: polygon 체인 (BOB 배포)

    // list_sell() -> create_escrow
    let tx = await sellerVault.list_sell(order, _input_sig);
    await tx.wait();
    console.log(tx);
    console.log(order);

    let recoverAddr = await sellerVault.order_recover(order, chain_id, nonce, _input_sig);
    console.log(recoverAddr);

    tx = await orderFactory.create_escrow(order, chain_id, nonce, _input_sig);
    await tx.wait();
    console.log(tx);
    console.log(order);
    ...
    // seller
    // resolve_sell(외부 체인) -> resolve_escrow(polygon) (HyperLane)
    tx = await sellerVault.resolve_sell(nonce, resolveSig, {value: eth_amount});
    await tx.wait();
    console.log(tx);

    tx = await bob.transfer(escrowAddr, bob_amount); 
    await tx.wait();
    console.log(tx);
